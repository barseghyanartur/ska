...
- `DEFAULT_SIGNATURE_PARAM` (str): Default name of the REQUEST param holding the generated signature value.
  Default value is `signature`.
- `DEFAULT_AUTH_USER_PARAM` (str): Default name of the REQUEST param holding the ``auth_user`` value.
  Default value is `auth_user`.
- `DEFAULT_VALID_UNTIL_PARAM` (str): Default name of the REQUEST param holding the ``valid_until`` value.
  Default value is `valid_until`.
- `DEFAULT_TIME_ZONE_PARAM` (str): Default name of the REQUEST param holding the ``time_zone`` value.
  Default value is `time_zone`.
- `DEFAULT_EXTRA_PARAM` (str): Default name of the REQUEST param holding the ``extra`` value.
  Default value is `extra`.
- `DEFAULT_PROVIDER_PARAM` (str): Default name of the REQUEST param holding the ``provider`` value.
...
__all__ = (
    ...
    'DEFAULT_VALID_UNTIL_PARAM', 'DEFAULT_TIME_ZONE_PARAM', 'DEFAULT_EXTRA_PARAM',
    'DEFAULT_PROVIDER_PARAM', 'DEFAULT_URL_SUFFIX', 'DEFAULT_RESERVED_PARAMS', 'DEBUG'
    )
...
# A list of REQUEST params reserved by default. Users should not be allowed to use them.
